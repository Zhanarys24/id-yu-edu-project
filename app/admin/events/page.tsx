...
// API для получения приложений
const ApplicationsApi = {
  getApplications: async (): Promise<ApiApplication[]> => {
    const token = getAuthToken();
    
    if (!token) {
      console.warn('Токен авторизации не найден, используем локальные данные');
      return [];
    }

    try {
      const response = await fetch('https://id.yu.edu.kz/api/v1/site_applications/', {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        console.warn(`API error! status: ${response.status}, используем локальные данные`);
        return [];
      }

      const data: ApiApplicationsResponse = await response.json();
      console.log('Получено приложений из API:', data.results.length);
      return data.results;
    } catch (error) {
  ...
  const loadCards = async () => {
    ...
    try {
      ...
      if (apiApps.length > 0) {
        ...
        // Если не удалось получить из API, используем локальные данные
