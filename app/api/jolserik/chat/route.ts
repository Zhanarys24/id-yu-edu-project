...
Что интересует? 😊`,
    ...
    // Добавляем быстрые ответы для справок
    'справка': `📄 **Справки и документы**

📋 **Популярные справки:**
• Справка об обучении (500₸, 3 дня)
• Академическая справка (1000₸, 5 дней)  
• Справка о стипендии (бесплатно, 1 день)

📞 **Деканат:** +7 (7292) 40-01-01
📍 **2 этаж, каб. 201-205**

💡 Какая именно справка нужна?`,

    'как получить справку': `📄 **Получение справки об обучении:**

1️⃣ Идите в деканат (2 этаж, каб. 201-205)
2️⃣ Возьмите с собой паспорт + студенческий билет  
3️⃣ Заполните заявление
4️⃣ Оплатите 500 тенге
5️⃣ Получите через 3 рабочих дня

📞 **Деканат:** +7 (7292) 40-01-01`,

    'документы': 'справка', // Редирект на справка
...
🚶‍♂️ **Как найти:** Главный вход → 2 этаж → направо`,

    'столовая': `🍽️ **Столовая университета**

📍 **Местоположение:** Главный корпус, 1 этаж
⏰ **Время работы:** 8:00-18:00
💳 **Оплата:** наличные, карты, студенческая карта`,

    'библиотека': `📚 **Библиотека**

📍 **Местоположение:** Учебный корпус, 2-3 этаж  
⏰ **Время работы:** 9:00-20:00
💻 **Услуги:** книги, интернет, тихие зоны для учебы`
  };

  const lowerMessage = message.toLowerCase().trim()
  
  // Проверяем точные и частичные совпадения
  for (const [keyword, response] of Object.entries(quickAnswers)) {
    if (lowerMessage === keyword || lowerMessage.includes(keyword)) {
      // Если это редирект, получаем конечный ответ
